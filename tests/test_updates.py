...
class UpdatesTest(unittest.TestCase):
    ...
    def test_contract_generator_requires_editor_login_in_page(self):
        ...
        self.assertIn("function calculateOptimalFreight(distance, totalHours, operation = {}, values = contractValues.estimateB)", page)
        ...
        self.assertIn("入力変更後の輸送書類再発行", page)
        ...
        self.assertIn(".individual-contract-page { height: var(--paper-height); overflow: hidden;", page)
        self.assertIn(".contract-page.individual-contract-page {", page)
        self.assertEqual(page.count('class="contract-page individual-contract-page'), 3)
        self.assertIn('id="printLayoutDensity"', page)
        self.assertIn("function fitIndividualContractLayout()", page)
        self.assertIn("page.scrollHeight <= page.clientHeight + 1", page)
        self.assertIn("layout-paginated", page)
        self.assertIn("partialPrintOptions.forEach(option => { option.disabled = true; })", page)
        self.assertIn("window.addEventListener('beforeprint', fitIndividualContractLayout)", page)
        ...
        self.assertIn("if (instrumentLookupRequests.has(requestKey))", page)
        self.assertIn("instrumentLookupRequests.delete(requestKey);", page)
        self.assertIn("requestRevision !== editorRevision", page)
        self.assertIn("const safeText = /^\\s*[=+\\-@]/.test(text)", page)
        ...
        self.assertIn("if (unitInput) unitInput.value = item.unit_value;", page)
        self.assertNotIn("taxIncludedPrice", page)
        self.assertIn("const result = await response.json();\n      const currentItem = contractValues.estimateB.cargo_items[rowIndex];", page)
        ...
        self.assertIn("route_origin: contractValues.estimateB.route_origin", page)
    ...
    def test_transport_sheet_api_uses_authenticated_apps_script(self):
        payload = {
            ...
            "cargo_items": [{"description": "チューバ", "quantity": "2", "unit_value": "800000", "volume_points": "8"}],
            "freight_rate_master": {"effective_date": "2026-08-23", "source_url": "https://example.com/rates", "verified": True},
        ...
        self.assertEqual(response.json["route_url"], send_request.return_value["routeUrl"])
    ...
    def test_transport_sheet_api_rejects_unconfirmed_or_invalid_cargo(self):
        base_payload = {
            "client_name": "テスト取引先",
            "transport_name": "楽器輸送",
            "editor_email": "music@example.com",
            ...
            "total_hours": "8",
            "freight_operation": {},
            "cargo_items": [{"description": "チューバ", "quantity": "1", "unit_value": "800000", "volume_points": "8"}],
            "freight_rate_master": {"effective_date": "2026-08-23", "source_url": "https://example.com/rates", "verified": True},
            "instrument_price_master": {},
        }
        with patch.dict(os.environ, {"EDITOR_PASSWORD": "editor-secret"}):
            client = create_app(database_url="").test_client()
            ...
            invalid_cargo = client.post(
                "/api/contracts/transport-sheet",
                headers={"X-Editor-Password": "editor-secret"},
                json={**base_payload, "cargo_restrictions_agreed": True, "cargo_items": [{"description": "チューバ", "quantity": "0", "unit_value": "-1", "volume_points": "8"}]},
            )

        self.assertEqual(no_consent.status_code, 400)
        self.assertIn("同意", no_consent.json["error"])
        self.assertEqual(invalid_cargo.status_code, 400)
        self.assertIn("輸送対象物", invalid_cargo.json["error"])
    ...
    def test_apps_script_caches_update_and_delete_results_for_safe_retry(self):
        ...
        self.assertIn('var lockedCachedResult = CacheService.getScriptCache().get("admin:" + requestId)', script)
    ...
    def test_apps_script_transport_sheet_includes_case_workflow(self):
        ...
        self.assertIn("file.setTrashed(true)", script)
    ...
    def test_lesson_page_is_available(self):
        ...
        self.assertIn("const controller = new AbortController()", page)
        self.assertIn("controller.abort(), 30000", page)
        self.assertIn("signal: controller.signal", page)
        self.assertIn("最大30秒ほどお待ちください", page)
        self.assertIn("window.setTimeout(resolve, 1200 * (attempt + 1))", page)
        self.assertIn("空き状況を取得できませんでした", page)
    ...
    def test_schedule_page_shares_public_calendar_and_admin_panel(self):
        ...
        self.assertIn('let adminToken = "";', page)
        self.assertIn('"X-Editor-Token": adminToken', page)
        self.assertIn('body: JSON.stringify({ editor_password: passwordInput.value })', page)
        self.assertIn('adminToken = result.editor_token;', page)
        self.assertIn('catch (error) { adminToken = ""; adminStatus.textContent = error.message; return; }', page)
        self.assertNotIn('"X-Editor-Password": adminPassword', page)
        ...
        self.assertIn('`要確認${statusRanges.length', page)
        ...
        self.assertIn('summary.textContent = isPastDate', page)
        self.assertIn('? "受付終了"', page)
    ...
    def test_index_provides_compact_monthly_updates_window_and_editor(self):
        ...
        self.assertIn("location.hostname === 'p4443.github.io'", page)
        self.assertIn("location.pathname.startsWith('/Namegawa-Brass-Lab')", page)
        self.assertIn("location.replace(`https://namegawa-brass-lab.com/", page)
        ...
        self.assertRegex(page, r"@media \(max-width: 600px\)[\s\S]*?\.update-media-preview\s*\{[^}]*height: 180px;")
    ...
    def test_index_presents_three_community_feature_rings(self):
        ...
        self.assertIn('href="video/?v=20260817-1"', page)
        self.assertIn('aria-label="みんなの居場所づくりサポートの動画を見る"', page)
        self.assertIn("居場所づくり動画を見る ▶", page)
