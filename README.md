...
契約書作成画面のB見積書では、`Code.gs` v24の`generate_transport_sheet`を使い、輸送対象物明細と料金規定を同じGoogleスプレッドシートへ発行します。生成ファイルは一般公開せず、画面で指定した共有先メールアドレスだけを編集者へ追加します。
...
開始時刻は、体験レッスン・小学生が毎時00分または30分、中学生・高校生以上が毎時00分です。「要相談」の枠はこの制限にかかわらず個別調整します。
