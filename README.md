...
契約書作成画面でB契約前見積書を選び、「距離・時間から参考運賃を算出」から出発地・目的地と片道・往復区分を入力して距離を測定します。Google Maps Routes APIの片道測定値を保存し、片道ではその値を、往復では2倍した値を見積距離へ反映します。住所または区分変更後は再測定が必要です。参考所要時間には荷役・待機時間が含まれないため、総拘束時間は案件条件に合わせて別途入力してください。入力途中の計算失敗では見積値を変更せず、発行後に入力を変えた場合は古いシートURLを無効扱いにして再発行を案内します。

楽器価格の照会は、登録済みURL最大7件を一括取得せず、各輸送対象物でユーザーが選択した公式カタログ1件だけを対象にします。型番は完全一致に限定せず、全角・半角、大文字・小文字、空白、ハイフン、末尾記号や世代表記の差を正規化し、表記一致、正規化一致、近似候補の順で表示します。候補は自動確定せず、掲載型番と価格を確認して選択します。選択したカタログ表示価格は税計算せず、そのまま評価単価として使用します。

### Brass-Logi輸送明細シート

契約書作成画面のB見積書では、`Code.gs`の`generate_transport_sheet`を使い、輸送対象物明細、運行計画・積卸し経路図、料金規定を同じGoogleスプレッドシートへ発行します。「輸送3シート・共有URLを自動作成」を実行すると、各シートの`gid`付きURLがB見積へ自動設定されます。経路シートには出発地、目的地、Google Mapsの片道測定値、片道・往復区分、見積反映距離、拘束時間、積卸し条件、Googleマップへのリンクを記録します。生成ファイルは一般公開せず、画面で指定した共有先メールアドレスへ編集権限を追加します。輸送対象外品への同意、正数の品目値、確認済み料金表、Google距離測定の証跡がない要求はAPIとApps Scriptの両方で拒否します。

実案件は次の順序で処理します。

1. `案件受付（下書き）`で取引先、経路、品目を登録します。この段階の印刷物は「見積準備票」で、金額は未確定です。
2. 楽器のメーカー・型番を入力し、選択した公式カタログから価格候補を照会して評価額を確定します。
3. Google Mapsで片道距離を測定し、片道・往復区分、自社料金、拘束時間、積卸し条件を反映します。積載上限を超える場合は品目を見直すか、自社軽貨物による分割運行にします。
4. 不足項目がないことを確認して`正式見積の発行準備完了`へ進め、輸送関連3シートを自動作成します。確定B見積からB契約書を作成すると、案件、金額、経路、3つの共有URLが引き継がれます。

国土交通省の「標準的な運賃」は参考額の確認にのみ使用します。顧客への請求額は、黒ナンバー軽貨物車による自社運行の届出運賃、案件条件、追加料金規定に基づいて確定します。

「2026年度 自社料金規定を読み込む」から距離制、時間制、待機、荷役、休日・深夜割増の料金表を利用できます。料金改定時は`lightCargoReferenceRateHistory`へ適用開始日付きで追加し、見積作成日以前に適用開始されたデータのうち最新のものを使用します。保存済み見積は選択時点の料金スナップショットを保持するため、後日の改定で過去の見積額は変わりません。利用前に出典URLと自社が運輸支局へ届け出た運賃を照合し、確認済みにしてください。
...
期間と15分枠の開始時刻を指定して、`予約済`または`お休み`を一括反映できます。開始時間と「最終枠の開始時間」はどちらも更新対象に含まれます。たとえば`09:00`から`10:00`は、`09:00` / `09:15` / `09:30` / `09:45` / `10:00`の5枠を更新します。レッスン表示の`09:00〜10:00（60分）`とは意味が異なり、このレッスンが使用する枠は`09:00`から`09:45`までの4枠です。予約の終了時刻を一括設定の最終枠として入力しないでください。
...
- 土曜日と日曜日は予約者から「要相談」で受け付け、管理者は予約者一覧の時刻入力へ決定時刻を入れて状態と同時に更新できます。予約者一覧の変更では、開始時刻と所要時間から占有枠を再計算します。たとえば`09:00`・60分は`09:00`から`09:45`までの4枠です。
...
- `09:00` / `09:15` / `09:30` / `09:45` / `10:00` の5枠が `予約済` 表示かつ選択不可であることを確認。この一括設定はレッスンの`09:00〜10:00（60分）`と異なり、`10:00`開始枠も更新する。
